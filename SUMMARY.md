...
- **widget_paint_custom** — единый промежуточный слой для custom-paint виджетов. Автоматически обрабатывает фон, обводку, тени, padding/margin, state-атрибуты (`_hover`/`_click`/`_focus`). Каскадное наследование MenuBar → Menu → MenuItem.
