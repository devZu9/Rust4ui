...
> **Rust for UI** — собирай UI на Rust как конструктор: JSON / EFx → живой интерфейс на egui. Без перекомпиляции между правками. С возможностью «запечь» в Rust-код для релиза.
...
| **EFx-совместимость** | Те же виджеты, что в `efx!()` — кодогенерация по желанию |
