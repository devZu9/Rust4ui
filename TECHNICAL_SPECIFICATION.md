...
## 8. widget_base — единый слой отрисовки

`widget_base` — промежуточная функция, которая забирает на себя **общую для всех custom-paint виджетов логику**: alloc, фон, обводку, тени, padding, margin, rounding и state-атрибуты.

### 8.1. Мотивация

Каждый custom-paint виджет (Button, IconButton, MenuItem, ComboBox, ...) до widget_base содержал копипасту:
...
widget_base выносит это в одно место. Виджет остаётся ответственным только за **уникальный контент** (иконка, текст, стрелка, ...) и **свои специфические атрибуты** (color_icon, icon_size, align, ...).
...
MenuBar { background: "#000", color: "#0FF" }
  └─ Menu (наследует background от MenuBar, может переопределить color)
       └─ MenuItem (наследует background и color от Menu)
            └─ если не указано своё → берётся из inherited_bg
...
### 8.5. Пример: кастомный виджет на widget_base

```rust
use crate::widgets::base::widget_base;
...
pub fn render(ui: &mut egui::Ui, node: &serde_json::Value, ctx: &mut RenderCtx) {
    ...
    // 2. widget_base — alloc + фон + обводка + тени + состояние
    let out = widget_base(
...
Без widget_base этот же код потребовал бы ~70 строк (padding, margin, allocate, get_state_background, get_state_border, draw_border, ...). С widget_base — **20 строк**.

### 8.6. Какие виджеты используют widget_base
