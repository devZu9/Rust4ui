<!-- v1.0.0 (2026-08-04) TECH_SPECIFICATION.md -->
...
- При наведении (hover) справа появляются две стрелки ^ и Ў
- Стрелка ^ — увеличить на `step`, Ў — уменьшить на `step`
...
L-- README.md             # инструкция по добавлению своих иконок
```
...
## 8. widget_paint_custom — единый слой отрисовки

`widget_paint_custom` — промежуточная функция, которая забирает на себя **общую для всех custom-paint виджетов логику**: alloc, фон, обводку, тени, padding, margin, rounding и state-атрибуты.

### 8.1. Мотивация

Каждый custom-paint виджет (Button, IconButton, MenuItem, ComboBox, ...) до widget_paint_custom содержал копипасту:

```
get_padding > get_margin > расчёт размера > allocate_exact_size > Response
get_state_background > rect_filled
get_state_border > draw_border
get_state_attr("shadow_background") > draw_shadow_bg
get_state_attr("shadow_border") > draw_shadow_border
```

widget_paint_custom выносит это в одно место. Виджет остаётся ответственным только за **уникальный контент** (иконка, текст, стрелка, ...) и **свои специфические атрибуты** (color_icon, icon_size, align, ...).

### 8.2. API

```rust
pub struct BaseOut {
    pub response: egui::Response,   // результат alloc + hover/click/focus
    pub content_rect: egui::Rect,    // после margin, до padding
    pub inner_rect: egui::Rect,      // после padding — место для контента
    pub rounding_cr: egui::CornerRadius,
}

pub fn widget_paint_custom(
    ui: &mut egui::Ui,
    node: &serde_json::Value,         // JSON-узел виджета
    theme: &Theme,                    // текущая тема
    widget: &str,                     // имя для theme lookup (напр. "Button")
    content_size: egui::Vec2,         // размер контента ДО padding
    sense: egui::Sense,               // click / click_and_drag / hover
    enabled: bool,
    default_bg: egui::Color32,
    default_rounding: f64,
    default_pad: egui::Margin,
    inherited_bg: Option<egui::Color32>,  // каскад от родителя
) -> BaseOut
```

### 8.3. Что делает widget_paint_custom

1. Читает `padding` и `margin` из node > theme > default
2. Вычисляет полный размер: `content + padding + margin`
3. `allocate_exact_size` > получает `Response`
4. Читает `rounding` (state-aware через `get_state_attr` > `_hover`/`_click`/`_focus`)
5. Читает `shadow_background` (state-aware) > рисует тень фона
6. Читает `background` (state-aware через `get_bg` > `_hover`/`_click`/`_focus`) > заполняет rect
7. Читает `border` (state-aware) + `shadow_border` > рисует обводку с тенью
8. Читает `padding` (state-aware) > вычисляет `inner_rect`
9. Возвращает `BaseOut { response, content_rect, inner_rect, rounding_cr }`

### 8.4. Каскадное наследование (inherited_bg)

Параметр `inherited_bg` реализует цепочку MenuBar > Menu > MenuItem:

```
MenuBar { background: "#000", color: "#0FF" }
  L- Menu (наследует background от MenuBar, может переопределить color)
       L- MenuItem (наследует background и color от Menu)
            L- если не указано своё > берётся из inherited_bg
               > если inherited_bg=None > читается тема
                  > если нет в теме > default_bg
```

Приоритет разрешения цвета: **node** > **inherited_bg** > **theme** > **default**.

### 8.5. Пример: кастомный виджет на widget_paint_custom

```rust
use crate::widgets::base::widget_paint_custom;
use crate::renderer::{attr_str, attr_f64, resolve_text, RenderCtx};

pub fn render(ui: &mut egui::Ui, node: &serde_json::Value, ctx: &mut RenderCtx) {
    // 1. Содержимое (уникальное для виджета)
    let text = resolve_text(attr_str(node, "text").unwrap_or(""), ctx);
    let font_size = attr_f64(node, "size").unwrap_or(14.0) as f32;
    ...
    // 2. widget_paint_custom — alloc + фон + обводка + тени + состояние
    let out = widget_paint_custom(
        ui, node, &ctx.theme, "MyWidget",
        galley.size(),                     // content_size
        egui::Sense::click(), true,        // sense, enabled
        egui::Color32::from_rgb(0x30, 0x30, 0x30),  // default_bg
        6.0,                               // default_rounding
        egui::Margin::symmetric(10, 4),    // default_pad
        ctx.inherited_bg,                  // каскад от родителя
    );

    // 3. Контент внутри inner_rect
    let pos = egui::pos2(
        egui::Align::Center.align_size_within_range(galley.size().x, out.inner_rect.x_range()).min,
        egui::Align::Center.align_size_within_range(galley.size().y, out.inner_rect.y_range()).min,
    );
    ui.painter().galley(pos, galley, egui::Color32::WHITE);

    // 4. Обработка клика
    if out.response.clicked() {
        log::info!("MyWidget нажат!");
    }
}
```

Без widget_paint_custom этот же код потребовал бы ~70 строк (padding, margin, allocate, get_state_background, get_state_border, draw_border, ...). С widget_paint_custom — **20 строк**.

### 8.6. Какие виджеты используют widget_paint_custom

| Виджет | Размер до | Размер после | Экономия |
|--------|-----------|-------------|----------|
| Button | 205 строк | 164 строки | ?20% |
| IconButton | 151 строка | 101 строка | ?33% |
| MenuItem | 103 строки | 76 строк | ?26% |
...
## 11. API — как это выглядит в коде
...
См. полный пример в секции 11.1.
...
"{{label.status}}"
  ...
  ¦     L-- ищет "label.status" в активной локали
  ¦     L-- получает "Статус: {status_text}"
  ¦
  +-- Проход 2: StateRegistry
  ¦     L-- резолвит {status_text} > "Работаю"
  ¦     L-- получает "Статус: Работаю"
  ¦
  L-- Результат: ui.label("Статус: Работаю")
```
...
"{{btn.save}}"
  ...
  ¦
  L-- Результат: ui.button("Сохранить")
```
...
### 14.7. Переключение языка в рантайме
...
## 17. Демо-файлы (демонстрационный прототип)
...
### 17.2. ui.json — модульная структура
...
| `selected_device` | `usize` | `0` | ComboBox устрои?ство |
...
### 17.5. Тема по умолчанию (`demo/theme.json`)
...
## 18. Тестирование и контроль качества
...
¦   L-- plural.rs          # 2e. Plural-правила: ru/en/pl/uk, проверка чисел 0–199
...
L-- smoke.rs               # 6. Smoke: каждый виджет изолированно, без паники
```

### 18.2. Уровень 1 — Pre-flight валидация
...
### 18.3. Уровень 2 — Unit-тесты подсистем
...
### 18.4. Уровень 3 — Интеграционный тест (demo/ui.json)
...
### 18.5. Уровень 4 — Persistence (автосохранение)
...
### 18.6. Уровень 5 — Encoding (UTF-8 без BOM)
...
### 18.7. Уровень 6 — Smoke (каждый виджет изолированно)
